 ...
 * 거주 형태를 한글로 변환
 */
function convertLivingArrangement(living?: string, other?: string): string {
  if (!living) return "독거";
...
export function convertToElderCreate(
  ...
  const relationship =
    elder.relationship === "other" && elder.relationshipOther
  ...
  return {
    ...
    // timezone 정보(Z) 제거 - PostgreSQL TIMESTAMP WITHOUT TIME ZONE 호환
    begin_date: new Date().toISOString().replace("Z", ""),
    end_date: null,
