        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-violet-600 to-purple-600 rounded-2xl shadow-xl p-8 text-white">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-xl font-black">AI 케어 요약</h2>
                    <p className="text-sm opacity-90">{dailyData.date}</p>
                  </div>
                </div>
              ...
              {/* 워드 클라우드 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">주요 키워드 (워드 클라우드)</h3>
                <div className="relative h-64 flex items-center justify-center">
                  <div className="absolute inset-0 bg-gradient-to-br from-violet-50 to-purple-50 rounded-xl"></div>
                  <div className="relative flex flex-wrap items-center justify-center gap-4 p-6">
                    {dailyData.keywords.map((kw, idx) => {
                      const sizes = ['text-4xl', 'text-3xl', 'text-2xl', 'text-xl'];
                      const colors = ['text-violet-600', 'text-purple-600', 'text-blue-600', 'text-pink-600'];
                      return (
                        <div
                          key={idx}
                          className={`${sizes[Math.min(idx, sizes.length - 1)]} ${colors[idx % colors.length]} font-black hover:scale-110 transition-transform cursor-pointer opacity-80 hover:opacity-100`}
                          style={{ 
          ...
          {activeTab === 'weekly' && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-2xl font-black text-slate-900 mb-1">{weeklyData.period}</h2>
                    <p className="text-sm text-slate-600">주간 케어 리포트</p>
                  </div>
              ...
              {/* 감정 추이 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">주간 감정 추이</h3>
              ...
              {/* 워드 클라우드 - 주간 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">주간 주요 키워드 (워드 클라우드)</h3>
                <div className="relative h-72 flex items-center justify-center">
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-50 via-violet-50 to-purple-50 rounded-xl"></div>
                  <div className="relative flex flex-wrap items-center justify-center gap-6 p-8">
                    {weeklyData.topKeywords.map((kw, idx) => {
                      const sizes = ['text-5xl', 'text-4xl', 'text-3xl', 'text-2xl', 'text-xl'];
                      const colors = ['text-violet-700', 'text-purple-700', 'text-blue-700', 'text-pink-700', 'text-indigo-700'];
                      return (
                        <div
                          key={idx}
                          className={`${sizes[Math.min(idx, sizes.length - 1)]} ${colors[idx % colors.length]} font-black hover:scale-125 transition-transform cursor-pointer opacity-80 hover:opacity-100`}
                          style={{ 
                        ...
                        >
                          {kw.word}
                          <span className="text-xs ml-2 opacity-70">×{kw.count}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              {/* 주요 키워드 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">주요 키워드 (전주 대비)</h3>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {weeklyData.topKeywords.map((kw, idx) => (
                    <div key={idx} className="text-center p-4 rounded-xl bg-slate-50 border border-slate-200 hover:shadow-md transition-all">
                      <p className="text-2xl font-black text-slate-900 mb-1">{kw.word}</p>
                      <p className="text-xs font-bold text-slate-600 mb-2">×{kw.count}</p>
          ...
          {activeTab === 'monthly' && (
            <div className="space-y-6">
              ...
              {/* 하이라이트 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">이번 달 하이라이트</h3>
                <div className="space-y-3">
                  {monthlyData.highlights.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-4 rounded-xl bg-green-50 border border-green-200">
              ...
              {/* 관심 사항 */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-lg font-black text-slate-900 mb-6">관심이 필요한 부분</h3>
                <div className="space-y-3">
                  {monthlyData.concerns.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
